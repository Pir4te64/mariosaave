...
const PlanCard = ({ title, subtitle, features, buttonText, link }) => {
  ...
  return (
    <div className='bg-white border border-greenmusgo rounded-lg shadow-sm flex flex-col p-4 h-[500px]'>
      ...
      {/* Botón fijo al fondo como link */}
      <a
        href={link}
        target='_blank'
        rel='noopener noreferrer'
        className='bg-greenmusgo hover:bg-softYellow hover:text-black text-white font-semibold py-2 px-4 mt-4 rounded-full text-center'>
        {buttonText}
      </a>
    </div>
  );
